...
-- Recreate the function with reward logic
...
BEGIN
  ...
  IF v_new_usage <= v_quota_limit THEN
    ...
    -- Check if user just completed daily quota (reached exactly 13)
    IF v_current_usage = v_quota_limit THEN
      ...
      IF v_reward_amount > 0 THEN
        ...
        INSERT INTO public.energy_transactions (user_id, transaction_type, amount, description)
        VALUES (v_user_id, 'reward', v_reward_amount, 'Daily completion reward');
